...
diffusion groups, using manifold-appropriate distance metrics:
  ...
  Translation (R³)
    Euclidean L2 distance between ligand centroids.
  Rotation (SO(3))
    Geodesic angle of the Kabsch best-fit rotation, symmetry-corrected via
    spyrmsd.
  Torsion (T^k)
    RMS wrapped angular difference across rotatable bonds, symmetry-corrected.

TARP reference distributions match DiffDock's randomize_position at t=1:
  Translation : N(Cα_COM, σ_tr² I).
  Rotation    : Uniform on SO(3) via random unit quaternion.
  Torsion     : Uniform on [−π, π]^k per bond.
"""
...
def get_rotatable_bonds(mol):
    ...
    components each with ≥ 2 atoms (non-ring, non-terminal).

    Args:
        mol: RDKit Mol (heavy atoms, no Hs).

    Returns:
        List of (n0, a, b, n1) atom-index 4-tuples for rdMolTransforms dihedral.
    """
    G = nx.Graph()
...
def rms_torsion_distance(angles1, angles2):
    ...
    Returns:
        float ≥ 0 in radians. NaN if arrays are empty or incompatible.
    """
...
def _apply_permutation(coords, idx1, idx2):
    ...
    Args:
        ...
        idx1: list/array of crystal atom indices.
        idx2: list/array of sample atom indices.

    Returns:
        (N, 3) array reordered to align crystal atom ordering.
    """
...
def _get_sym_permutations(crystal_centered, sample_centered_list, spy_mol,
                           timeout=_SYMM_TIMEOUT):
    ...
    Returns:
        List of (idx1, idx2) tuples, one per sample.
    """
...
def compute_group_distances(crystal_mol, crystal_coords, sample_coords_list, rot_bonds):
    ...
    Returns:
        dict with keys 'translation', 'rotation', 'torsion_rms',
        'torsion_per_bond', 'n_rot_bonds'.
    """
...
def compute_tarp_fractions_torsion(crystal_angles, sample_angles_list, K, rng):
    ...
    Args:
        ...
        sample_angles_list: list of (n_bonds,) symmetry-corrected arrays.
        K: number of random reference draws.
        rng: numpy Generator.
...
# ---------------------------------------------------------------------------
# Multiprocessing workers and batch runners
# ---------------------------------------------------------------------------
...
def run_group_tarp_eval(
    ...
    Returns:
        dict with keys 'translation', 'rotation', 'torsion' (each (n_valid, K)),
...
def run_group_distances(
    ...
    Returns:
        dict with keys 'names', 'n_rot_bonds', 'translation', 'rotation',
        'torsion_rms'. Distance arrays have shape (n_valid, S_max).
    """
    complex_names = list(complex_names)
    n = len(complex_names)

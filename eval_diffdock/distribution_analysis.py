...
# ---------------------------------------------------------------------------
# Internal helpers replacing DiffDock's utils.visualise dependencies
# ---------------------------------------------------------------------------

def _load_sdf_paths(results_dir: str) -> List[str]:
    """Return rank*.sdf paths sorted by rank number."""
    all_sdfs = glob.glob(os.path.join(results_dir, "rank*.sdf"))
    def _rank_key(p):
        m = re.search(r"rank(\d+)", os.path.basename(p))
        return int(m.group(1)) if m else 0
    return sorted(all_sdfs, key=_rank_key)


def _find_receptor_pdb(results_dir: str) -> Optional[str]:
    """Try to find the receptor PDB for a DiffDock results directory.

    Checks input_metadata.json first, then falls back to a glob for *.pdb.
    """
...
def load_poses(results_dir: str) -> List[PoseRecord]:
    ...
    """
    sdf_paths = _load_sdf_paths(results_dir)
    poses = []
...
def compute_rmsd_to_crystal(
    ...
    Args:
        crystal_sdf: Path to the ground-truth ligand SDF.
...
def plot_confidence_vs_rmsd(
    ...
    Each point is one predicted pose. Spearman ρ is annotated; a negative
    value means confidence correctly ranks accurate poses higher.

    Args:
        poses: List of PoseRecord from ``load_poses``.
        rmsds: RMSD array from ``compute_rmsd_to_crystal``.
        ax: Optional existing matplotlib Axes.
        rmsd_threshold: Horizontal dashed line marking the success threshold.
...
def saturation_analysis(
    ...
    Args:
        poses: List of PoseRecord (assumed rank-sorted).
...
def view_poses_colored(
    ...
    Args:
        ...
        receptor_pdb: Path to receptor PDB. If None and results_dir is given,
            inferred from input_metadata.json or a *.pdb glob.
        results_dir: DiffDock output directory (used to find receptor PDB).
        color_by: ``'confidence'`` or ``'cluster'``.
    ...
    if receptor_pdb is None and results_dir is not None:
        receptor_pdb = _find_receptor_pdb(results_dir)
    ...
    if color_by == "confidence":
        ...
        colours = [mcolors.to_hex(cmap(norm(c))) for c in confidences]
    ...
    for i, (pose, colour) in enumerate(zip(poses, colours)):
        pdb_block = Chem.MolToPDBBlock(pose.mol) or ""
...
def run_full_analysis(
    ...
    Args:
        ...
        crystal_sdf: Optional path to the ground-truth ligand SDF.

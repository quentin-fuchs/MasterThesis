...
def _mira_one_complex(crystal: np.ndarray, samples: list, num_runs: int,
    ...
    Args:
        ...
        device: torch device.
        metric: distance metric passed to MIRA.
            "euclidean" — Euclidean distance in flat q-space, equivalent to
                          scaled RMSD (√n_atoms × RMSD). Current default.
    ...
    if metric not in ("euclidean", "rmsd"):
        raise ValueError(f"metric must be 'euclidean' or 'rmsd', got {metric!r}")
...
def compute_mira_scores(
    ...
    Args:
        ...
        metric: "euclidean" (default, scaled RMSD) or "rmsd" (per-atom RMSD).

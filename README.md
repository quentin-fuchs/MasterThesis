...
The `molcalib` package is the core library and can be installed standalone:
...
|---|---|
| `molcalib.mira.mira_score(crystal_mol, crystal_coords, sample_coords, ca_coords, ...)` | MIRA score for one complex (symRMSD-based) |
| `molcalib.mira.mira_null(S)` | Null reference score for S samples (≈ 0.683 for S=40) |
| `molcalib.mira.bootstrap_mira_groups(names, scores, groups, ...)` | Bootstrap confidence intervals per protein group |
| `molcalib.tarp.tarp_fractions(crystal_mol, crystal_coords, sample_coords, ca_coords, K, ...)` | TARP fractions vector of length K for one complex |
| `molcalib.tarp.ecp_from_fractions(f_matrix)` | ECP curve from a matrix of TARP fractions |
| `molcalib.tarp.plot_ecp(ecp, alpha, ax, ...)` | Plot ECP with calibration diagonal |
| `molcalib.distances.compute_rmsd_symmetry(ref_coords, sample_coords, mol, ...)` | Symmetry-corrected RMSD via spyrmsd (4 s timeout) |
| `molcalib.prior.generate_reference_coords(template_mol, rot_bonds, ca_coords, ...)` | Draw one reference pose from the prior |

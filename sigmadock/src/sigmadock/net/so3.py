...
from sigmadock.net.wigner import wigner_D
...
class CoefficientMappingModule(torch.nn.Module):
    ...
    def complex_idx(
        self, m: int, lmax: int, m_complex: list[int], l_harmonic: list[int]
    ...
    def coefficient_idx(self, lmax: int, mmax: int) -> torch.Tensor:
        if (self.lmax_cache is not None) and (self.mmax_cache is not None):  # noqa: SIM102
            if (self.lmax_cache == lmax) and (self.mmax_cache == mmax):  # noqa: SIM102
...
class SO3_Embedding:
    ...
    def set_embedding(self, embedding: list[object]) -> None:
    ...
    def _rotate_inv(self, SO3_rotation: object, mappingReduced: object) -> None:
        if self.num_resolutions == 1:
    ...
    def _grid_act(self, SO3_grid: object, act: Callable, mappingReduced: object) -> None:
        offset = 0
        for i in range(self.num_resolutions):
            num_coefficients = mappingReduced.res_size[i]
    ...
    def to_grid(self, SO3_grid: object, lmax: int=-1) -> torch.Tensor:
        if lmax == -1:
            lmax = max(self.lmax_list)
    ...
    def _from_grid(self, x_grid: torch.Tensor, SO3_grid: object, lmax: int = -1) -> None:
        if lmax == -1:
            lmax = max(self.lmax_list)
...
class SO3_Rotation(torch.nn.Module):
    ...
    # Rotate the embedding
    def rotate(self, embedding: object, out_lmax: int, out_mmax: int) -> torch.Tensor:
        out_mask = self.mapping.coefficient_idx(out_lmax, out_mmax)
        wigner = self.wigner[:, out_mask, :]
        return torch.bmm(wigner, embedding)

    # Rotate the embedding by the inverse of the rotation matrix
    def rotate_inv(self, embedding: object, in_lmax: int, in_mmax: int) -> torch.Tensor:
...
class SO3_Grid(torch.nn.Module):
    ...
    def get_to_grid_mat(self, device: str | torch.device) -> torch.Tensor:
        return self.to_grid_mat

    # Compute matrices to transform grid to irreps
    def get_from_grid_mat(self, device: str | torch.device) -> torch.Tensor:    
        return self.from_grid_mat

    # Compute grid from irreps representation
    def to_grid(self, embedding: object, lmax: int, mmax: int) -> torch.Tensor:
...
class SO3_Linear(torch.nn.Module):
    ...
    def forward(self, input_embedding: object, output_scale: torch.Tensor | None=None) -> object:
        out = []
...
class SO3_LinearV2(torch.nn.Module):
    ...
    def forward(self, input_embedding: object) -> object:

...
from sigmadock.net.activation import (
    GateActivation,
    S2Activation,
    SeparableS2Activation,
    SmoothLeakyReLU,
)
from sigmadock.net.drop import EquivariantDropoutArraySphericalHarmonics, GraphDropPath
from sigmadock.net.layer_norm import get_normalization_layer
from sigmadock.net.radial_function import RadialFunction
from sigmadock.net.so2_ops import SO2_Convolution
from sigmadock.net.so3 import SO3_Embedding, SO3_LinearV2
...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(  # noqa: C901
        ...
        if self.use_m_share_rad:
            ...
            if self.share_edge_mlp:
                self.rad_func = RadialFunction(self.edge_channels_list)
            else:
                self.rad_func = nn.ModuleList()
                for i in range(num_edge_types):
                    bias = i not in HPARAMS.get_edge_group_indices("has_cutoff")
                    self.rad_func.append(RadialFunction(self.edge_channels_list, bias=bias))
    ...
    def forward(  # noqa: C901
        ...
        if self.use_m_share_rad:
            if self.share_edge_mlp:
                ...
                x_edge_weight = torch.zeros([len(x_edge), self.edge_channels_list[-1]], device=x_edge.device)
                for i in range(self.num_edge_types):
                    x_edge_weight[edge_entity == i] = self.rad_func[i](x_edge[edge_entity == i])
        ...
        if self.use_s2_act_attn:
            x_message = self.so2_conv_1(x_message, x_edge, edge_entity)
        else:
            x_message, x_0_extra = self.so2_conv_1(x_message, x_edge, edge_entity)
        ...
        if self.use_s2_act_attn:
            x_message, x_0_extra = self.so2_conv_2(x_message, x_edge, edge_entity)
        else:
            x_message = self.so2_conv_2(x_message, x_edge, edge_entity)
        ...
        if alpha.device.type == "mps":
            device = alpha.device
            alpha = torch_geometric.utils.softmax(alpha.cpu(), edge_index[1].cpu())
...
class TransBlockV2(torch.nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,  # SO3_Embedding
        edge_features: torch.Tensor,
        edge_index: torch.Tensor,
        edge_entity: torch.Tensor,
        batch: Batch,  # for GraphDropPath
    ) -> torch.Tensor:  # SO3_Embedding
        output_embedding = x

        x_res = output_embedding.embedding
        output_embedding.embedding = self.norm_1(output_embedding.embedding)
        output_embedding = self.ga(output_embedding, edge_features, edge_index, edge_entity)

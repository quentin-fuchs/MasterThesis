...
class SO2_Convolution(torch.nn.Module):
    ...
    Args:
        ...
        extra_m0_output_channels (int): If not None, return `out_embedding` (SO3_Embedding)
            and `extra_m0_features` (Tensor).
    """
    ...
    def forward(self, x: torch.Tensor, x_edge: torch.Tensor, edge_entity: torch.Tensor) -> torch.Tensor:
        ...
        if self.rad_func is not None:
            if self.share_edge_mlp:
                ...
                _x_edge = torch.zeros([len(x_edge), self.edge_channels_list[-1]], device=x_edge.device)
                for i in range(self.num_edge_types):
                    _x_edge[edge_entity == i] = self.rad_func[i](x_edge[edge_entity == i])
                x_edge = _x_edge
        offset_rad = 0
